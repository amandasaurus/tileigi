...
fn intersection(p1: &Point<i32>, p2: &Point<i32>, border: &Border<i32>) -> (i32, i32) {
    let x1 = p1.x() as i64;
    let y1 = p1.y() as i64;
    let x2 = p2.x() as i64;
    let y2 = p2.y() as i64;
...
fn calculate_intersections(linestring: &LineString<i32>, border: &Border<i32>) -> LineBorderIntersection<i32> {
    ...
    let mut intersections: Vec<IntersectionOption<_>> = Vec::with_capacity(linestring.0.len() - 1);
...
fn clip_linestring_to_border(ls: Cow<LineString<i32>>, border: &Border<i32>) -> Option<Geometry<i32>> {
    match calculate_intersections(&ls, border) {
        ...
        LineBorderIntersection::Intersections(intersections) => {
            let mut lines: Vec<LineString<_>> = Vec::new();
            let mut curr_line: Vec<Point<_>> = Vec::new();
...
fn clip_multilinestring_to_border(mls: Cow<MultiLineString<i32>>, border: &Border<i32>) -> Option<Geometry<i32>> {
    let mut lines: Vec<LineString<_>> = Vec::with_capacity(mls.0.len());
    match mls {
        Cow::Borrowed(mls) => {
...
fn clip_to_border(geom: Cow<Geometry<i32>>, border: &Border<i32>) -> Option<Geometry<i32>> {
    match geom {
        Cow::Owned(geom) => {
            match geom {
...
pub fn clip_to_bbox(geom: Cow<Geometry<i32>>, bbox: &Bbox<i32>) -> Option<Geometry<i32>> {
